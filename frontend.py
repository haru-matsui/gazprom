...
def generate_concept_board_openrouter(region_name, cultural_code):
    ...
    # Формируем строгий структурированный промпт
    prompt = (
        f"A professional, minimalist architectural interior and exterior design moodboard (presentation collage) for an industrial facility. "
        f"The image content must be strictly divided into a 3-section geometric grid layout. "
        f"No buildings, no fields, no sky, no natural landscapes. "
        f"SECTION 1 (TOP HALF): A flat, close-up abstract industrial pattern of a modern factory facade envelope, architectural blueprint scheme, and high-tech sandwich-panel modular grids matching the style: '{style}'. "
        f"SECTION 2 (BOTTOM LEFT QUADRANT): A macro close-up photography of physical building material textures: {materials}. "
        f"SECTION 3 (BOTTOM RIGHT QUADRANT): Three solid, clean rectangular color swatches representing the official corporate color palette: {colors}. "
        f"CRITICAL LAYOUT RULES: Use thin white lines to perfectly separate the sections. The composition must be flat, corporate, top-view product design catalog style. "
        f"CRITICAL TEXT RULES: Include exact, clean, readable text labels printed strictly in RUSSIAN language with Cyrillic alphabet. "
        f"Label the top section as 'ПРОМЫШЛЕННЫЙ ПАТТЕРН', label the bottom-left section as 'МАТЕРИАЛЫ', label the bottom-right section as 'ЦВЕТОВАЯ ПАЛИТРА'. "
        f"No English words, no gibberish text, professional graphic design typography."
    )
    # 3. Собираем запрос
    headers = {
        "Authorization": f"Bearer {api_key}",
        "HTTP-Referer": "https://localhost",  # OpenRouter просит указывать реферер
        "X-Title": "Scoring App"
    }
    ...
    payload = {
        ...
        "modalities": ["image"] # Указываем, что ждем картинку, а не текст
       # "seed": seed_value
    }
    ...
    if response.status_code == 200:
        ...
        # OpenRouter возвращает картинки по-разному в зависимости от модели
        # Вариант А: Изображение лежит в специальном массиве images
        ...
        # Вариант Б: Изображение вшито прямо в текст (markdown)
        if not image_url_or_b64 and "content" in message:
            content = message["content"]
            if "data:image/" in content:
                # Извлекаем Base64 строку из текста
        ...
        # 6. Конвертируем полученный Base64 обратно в картинку PIL
        ...
        # Полезно для дебага, если OpenRouter вернет ошибку
...
# Инициализируем словари в session_state, если их еще нет
if "concept_boards" not in st.session_state:
    st.session_state["concept_boards"] = {}  # Ключом будет индекс или имя региона

if "four_renders" not in st.session_state:
    st.session_state["four_renders"] = {}  # Для 4 ракурсов
    ...
    if not top_regions:
        ...
        for i, r in enumerate(top_regions):
            ...
            with tabs[i]:
                ...
                # Проверяем, есть ли культурный код у региона
                cultural_code = region_data.get("cultural_code")

                if cultural_code:
                    # Показываем палитру из HEX кодов для наглядности
                    colors_html = "".join([
                    ...
                    if st.button(f"Сгенерировать концепт для {region_data['name']}", key=f"generate_btn_{i}"):
                        with st.spinner("Nano Banana анализирует стиль и генерирует рендер..."):
                            try:
                                ...
                                # Если картинка для этой вкладки уже была сгенерирована ранее — просто показываем её
